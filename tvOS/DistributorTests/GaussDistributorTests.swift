...
class GaussDistributorTests: XCTestCase {
	...
	func testDerivate() {
		...
		do {
			...
			let la_gμ: la_object_t = la_matrix_from_float_buffer(zip(c.μ.buf, c.σ.buf).map {
				exp( -0.5 * ( $0.0 * $0.0 ) / ( $0.1 * $0.1 ) ) * rsqrt(2.0*Float.pi) / $0.1
			}, la_count_t(width), 1, 1, hint, attr)
			let la_gσ: la_object_t = la_matrix_from_float_buffer(zip(c.μ.buf, c.σ.buf).map {
				exp( -0.5 * ( $0.0 * $0.0 ) / ( $0.1 * $0.1 ) ) * rsqrt(2.0*Float.pi) / $0.1 / $0.1 * -$0.0
			}, la_count_t(width), 1, 1, hint, attr)
			...
			let la_Δ: la_object_t = la_matrix_from_float_buffer([
				...
				].reduce(la_splat_from_float(0, attr), la_sum).array.map{Float($0)},
			                                                     la_count_t(width), 1, 1, hint, attr)
