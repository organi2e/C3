...
let storage: URL = FileManager.default.temporaryDirectory.appendingPathComponent("MNISTv2.4994.sqlite")
	...
	func testMNIST() {
		do {
			...
			if try context.fetch(label: "I", width: 28 * 28).isEmpty {
				...
				let G: Cell = try context.make(label: "G", width: 384, distribution: .Gauss, activation: .Binary, adapters: (.Regular, .RegFloor), input: [H])
//				let F: Cell = try context.make(label: "F", width: 256, distribution: .Gauss, activation: .Binary, adapters: (.Regular, .RegFloor), input: [G])
				let _:  Cell = try context.make(label: "O", width: 10, distribution: .Gauss, activation: .Binary, adapters: (.Regular, .RegFloor), input: [G])
				try context.save()
			}
			do {
